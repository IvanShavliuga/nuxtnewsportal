...
export default createStore({
  ...
  mutations: {
    ...
    LIKEPOST(state, post) {
      ...
      if (lu[0] !== undefined)
        post.like = post.like.filter((idl) => {
          return idl !== state.userloginid;
        });
    ...
    REPOST(state, post) {
      ...
      if (lu[0] !== undefined)
        post.repost = post.repost.filter((idl) => {
          return idl !== state.userloginid;
        });
  ...
  actions: {
    ...
    fetchAll({ commit }) {
      api
        .fetchUsersList()
        .then((r) => r.data)
        .then((arr) => commit("GETUSERS", arr));
      api
        .fetchGroupsList()
        .then((r) => r.data)
        .then((arr) => commit("GETGROUPS", arr));
      api
        .fetchMessagesList()
        .then((r) => r.data)
        .then((arr) => commit("GETMESSAGES", arr));
      api
        .fetchCardsList()
        .then((r) => r.data)
        .then((arr) => commit("GETCARDS", arr));
      api
        .fetchPostsList()
        .then((r) => r.data)
        .then((arr) => commit("GETPOSTS", arr));
      api
        .fetchCommentsList()
        .then((r) => r.data)
        .then((arr) => commit("GETCOMMENTS", arr));
      api
        .fetchAlert()
        .then((r) => r.data)
        .then((arr) => commit("GETALERT", arr));
    ...
    postsUser: (state) => {
      ...
      for (let i = 0; i < state.posts.length; i++) {
        ...
        if (gi === -1) {
          ...
          if (
            frid[0] !== undefined ||
            state.posts[i].userId === state.userloginid
          )
        ...
        if (ru[0] !== undefined) {
          ...
          state.groups[gi].idAdmin !== state.userloginid &&
          fl[0] == state.userloginid &&
          pf === false
        ) {
          pu.push(state.posts[i]);
        } else if (
